...
Phew! I'm glad to be done with the [100 Days of Gatsby](https://twitter.com/gatsbyjs/status/1208068118335119364) forms challenge. Mostly because... hooray! It's time to work with [MDX](https://mdxjs.com/) in Gatsby. One of my favorite parts of the [JAMstack](https://jamstack.org/). I used MDX heavily when I was Tech Lead for O'Reilly Media's Design System documentation site, so I've been familiar with the syntax and the power of this specification. Considering it's been a year since I started officially using MDX and working with GatsbyJS, I've been curious to learn what's changed with how Gatsby makes MDX available with [gatsby-plugin-mdx](https://www.gatsbyjs.org/packages/gatsby-plugin-mdx/). Here's what I learned from following the prompts of this [latest Gatsby challenge](https://www.gatsbyjs.org/blog/100days/mdx/).

## Mix Markdown and React using MDX

Adding `gatsby-plugin-mdx` and all the companion dependencies allowed me to render any MDX files in my site with no issue. This section on [writing pages in MDX](https://www.gatsbyjs.org/docs/mdx/writing-pages/) in Gatsby was clear:
...
> **Fun fact:** You can make a component using an MDX file and import it into another MDX file! Make sure you do a default export in your MDX-based component, though, to make sure it isn’t wrapped by the `defaultLayout` template set in your `gatsby-plugin-mdx` options in your `gatsby-config.js` file
...
Nesting MDX files inside of MDX files are interesting, though. If you pass in an MDX file into another, you can’t pass children into that imported MDX file like you can do with a standard React component.
...
There seems to be a lot of features under the hood for rendering MDX files when those files are hosted in the `src/pages` folder. For example, it seems you don’t **need** to wrap the contents of your layout component with the `<MDXProvider>` component anymore. I had to do that with my layout components a year ago. Very nice.
...
I discovered it’s because that `posts/` folder is nested in the `src/` folder. It looked like any files in the `src/pages` folder would be grabbed and rendered by Gatsby, as long as I used `gatsby-plugin-page-creator` to point to a subfolder. This made me curious, so I tried something else.
