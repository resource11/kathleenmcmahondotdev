...
Next, I went through the part 1 steps, and made pages with images for the Markdown pages. I got a bit stuck when trying to do the same with [MDX](https://mdxjs.com/) files, and remembered that you need to use the `MDXProvider` to wrap your `Layout` when using MDX. I want to see if you can do both Markdown and MDX queries at the same time in a page, so I tested that out and it works.

While reading the [pages and layouts recipes](https://www.gatsbyjs.org/docs/recipes/pages-layouts/#creating-pages-automatically) docs section, I remembered the `createPage()` method uses the `path` value to create the route, the `component` value to use the template, and the `context` value to set the context object. In the [provided docs example](https://www.gatsbyjs.org/docs/recipes/pages-layouts/#directions-3) the `createPages` method used the `forEach()` method to loop through the `dogData` array of objects to provide the `path` `component` and `context` value for each `dog`.

Then, in your template, you could access the `dog` value in your page component by destructuring the `pageContext` object like so:
